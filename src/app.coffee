...
class SimpleConsumer
  constructor: ({ @autoCommit, @batch, @batchSize, @batchWait, @brokers, @group, @topic }) ->
    ...
    @batch = false unless @batch is true # queue msgs in batches or process as they arrive?
  ...
  subscribe: (callback) =>
    unless @batch # process individual messages
    ...
    @consumer.on 'ready', =>
      console.log "Consumer #{@group}/#{@topic} running..."
      @consumer.subscribe [@topic]
      setInterval =>
        @consumer.consume @batchSize, (err, messages) =>
          throw err if err?
          if @batch and messages.length > 0 # process batched array of messages
            parsed = []
            for msg in messages
